...
extern "C" {
     ...
     * */
    int             adp_cnt_offer(adp_cnt_ctx_t *ctx, const void *buf,
                                  uint32_t len);
     ...
     * @retval -1 If error occured.
     *
     * @see adp_cnt_merge, adp_cnt_merge_bytes, adp_cnt_get_bytes
     * */
    int             adp_cnt_get_raw_bytes(adp_cnt_ctx_t *ctx, void *buf,
                                          uint32_t *len);
     ...
     * @retval -1 If error occured.
     *
     * @see adp_cnt_merge, adp_cnt_merge_bytes, adp_cnt_get_raw_bytes
     * */
    int             adp_cnt_get_bytes(adp_cnt_ctx_t *ctx, void *buf,
                                      uint32_t *len);
    ...
    /**
     * Merge several adaptive counting raw bitmaps into the current context,
     * effectively combined all distinct countings.
     *
     * Usage:
     * @code{c}
     * if(adp_cnt_merge_raw_bytes(ctx, buf_1, len_1, buf_2, len_2,
     *     ..., buf_n, len_n, NULL)) {
     ...
     * @retval -1 if error occured.
     *
     * @see adp_cnt_merge, adp_cnt_get_raw_bytes, adp_cnt_merge_bytes
     * */
    int             adp_cnt_merge_raw_bytes(adp_cnt_ctx_t *ctx,
                                            const void *buf, uint32_t len,
                                            ...);
    /**
     * Merge several adaptive counting bitmaps into the current context,
     * effectively combined all distinct countings.
     *
     * Usage:
     * @code{c}
     ...
     * @retval -1 if error occured.
     *
     * @see adp_cnt_merge, adp_cnt_get_bytes, adp_cnt_merge_raw_bytes
     * */
    int             adp_cnt_merge_bytes(adp_cnt_ctx_t *ctx,
                                        const void *buf, uint32_t len,
                                        ...);
